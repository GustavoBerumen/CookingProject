...
kable(dur_ind, "latex", alig = "l", caption = "Average duration of items used", booktabs = T) %>%
  kable_styling(latex_options = c("hold_position")) %>%
   footnote(symbol = ("c = CPGs, u = utensils, e = environment")) %>%
   footnote(symbol = ("time in seconds"))
```

### Total and average used of items by subcategory
```{r, warning=FALSE, echo=FALSE, message=FALSE}

# get mean duration by category
count.cat <- reg.new.con.cat %>%
  dplyr::select(category, duration, type, p_corrected) %>% 
  dplyr::group_by(category, type, p_corrected) %>% 
  dplyr::summarise(total = length(category))

# subset c
count.cat.c <- count.cat[which(count.cat$type=='c'), ]
# round average time
count.cat.c[ ,4] <- round(count.cat.c[,4], digits =1)
 
# 
kable(count.cat.c[, c(1,3:4)], "latex", alig = "l", caption = "Total and average used of items by subcategory", booktabs = T) %>%
  kable_styling(latex_options = c("hold_position"))
```
