...
kable(source_df,  "latex", alig = "l", caption = "Source of recipes", booktabs = T) %>%
  kable_styling(latex_options = c("scale_down",  "hold_position")) %>%
   footnote(symbol = ("Notes. s.time = sugested cooking time in minutes, s. CPGs = suggested number of CPGs ingredients"))
```
...
kable(time.list, "latex", alig = "l", caption = "Duration of cooking per session", booktabs = T) %>%
    kable_styling(latex_options = c("hold_position")) %>%
    footnote(symbol = ("Notes. Time in minutes"))
```

### Comparison recipe vs actual new recipe

```{r, warning=FALSE, echo=FALSE, message=FALSE}
kable(comp.rec.new, "latex", alig = "l", caption = "Comparison of durations and CPGs (recipe vs actual recipe)", booktabs = T) %>%
    kable_styling(latex_options = c("hold_position")) %>%
    footnote(symbol = ("Notes. s = suggested in the recipe, Time in minutes"))
```
...
### Average duration by subcategory 
```{r, warning=FALSE, echo=FALSE, message=FALSE}

# get mean duration by category
pivot <- reg.new.con.cat %>%
  dplyr::select(category, duration, type) %>% 
  dplyr::group_by(category, type) %>% 
  dplyr::summarise(totalDuration = sum(duration), avgDur = mean(duration), length(category))

# subset c
pivot.c <- pivot[which(pivot$type=='c'), ]
# round average time
pivot.c[ ,4] <- round(pivot.c[,4], digits =1)


# 
kable(pivot.c[, c(1,4)], "latex", alig = "l", caption = "Average duration of items used by category", booktabs = T) %>%
  kable_styling(latex_options = c("hold_position")) %>%
   footnote(symbol = ("time in seconds"))
```
